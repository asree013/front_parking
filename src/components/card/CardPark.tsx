    ...
    <Card sx={{ maxWidth: 345 }} className='bg_card'>
      ...
      </CardContent>
      {
        ...
        : <CardActions style={{justifyContent: 'space-between'}}>
            ...
            <Button variant='contained' disabled={Boolean(data.active === false)} onClick={() => router.push(`/park/${data.id}/add/?start=${query.start_date}&end=${query.end_date}`)}  size="large">{data.active=== false? 'ห้ามจอง': 'จอง'}</Button>
          </CardActions>
      }
    </Card>
  );
